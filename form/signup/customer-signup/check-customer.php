...
if (isset($_POST['customerSubmit'])) {
    ...
    if (!empty($firstName) && !empty($lastName)) {
        ...
        if (!preg_match('~[0-9]~', $firstName) && !preg_match('~[0-9]~', $lastName)) {
            ...
            if (strlen($firstName) > 20 || strlen($lastName) > 20) {
                $errors['name'] = "<p class='text-danger  p-0 m-0 d-flex align-items-center '>Firstname and Lastname has limit of 20 letters long</p>";
            ...
            $errors['name'] = "<p class='text-danger p-0 m-0  d-flex align-items-center'>Firstname and Lastname cannot contain numbers</p>";
        }
    } else {
        $errors['name'] = "<p class='text-danger  p-0 m-0 d-flex align-items-center'>Firstname or Lastname cannot be empty</p>";
    }
    ...
    if (!empty($addr)) {
        ...
        $errors['address'] = "<p class='text-danger  p-0 m-0 d-flex align-items-center'>Address field cannot be empty</p>";
    }
    ...
    if (!empty($email)) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($count > 0) {
                $errors['email'] = "<p class='text-danger p-0 m-0 d-flex align-items-centerX'>Sorry, Email already exists.</p>";
            ...
            $errors['email'] = "<p class='text-danger p-0 m-0 d-flex align-items-centerX'>Given email is in invalid format</p>";
        }
    } else {
        $errors['email'] = "<p class='text-danger p-0 m-0 d-flex align-items-center'>Email field cannot be empty</p>";
    }
    ...
    if (!empty($phoneNum)) {
        if (preg_match("*^[0-9]+$*", $phoneNum)) {
            ...
            if (strlen($phoneNum) === 10) {
                ...
                if ($count > 0) {
                    $errors['phonenum'] = "<p class='text-danger p-0 m-0 d-flex align-items-centerX'>Sorry, Phone number already exists.</p>";
                } else {
                    $customerPhoneNum = $phoneNum;
                }

            } else {
                $errors['phonenum'] = "<p class='text-danger p-0 m-0 d-flex align-items-center'>Phone number should be 10 characters long</p>";
            }

        } else {
            $errors['phonenum'] = "<p class='text-danger p-0 m-0 d-flex align-items-center'>Given phone number is invalid</p>";
        }
    } else {
        $errors['phonenum'] = "<p class ='text-danger p-0 m-0 d-flex align-items-center'>Phone Number cannot be empty</p>";
    }
    ...
    if (!empty($password)) {
        ...
        $errors['password'] = "<p class='text-danger p-0 m-0 d-flex align-items-center'>Password field cannot be empty</p>";
    }
    ...
    if (empty($errors['name']) && empty($errors['address']) && empty($errors['email']) && empty($errors['phonenum']) && empty($errors['password']) && empty($errors['password_check'])) {
        ...
        $subject = get_customer_mail_signup("localhost/website/project/form/signup/customer-signup/verify-customer.php?token=$token");
        ...
        if (mail($sender, $body, $subject, $header)) {
            ...
            $mail_error_msg = "<div class='text-danger border border-danger p-1 text-center'>
                            <p class='mb-1 p-0'><i class='fas fa-check-circle text-danger m-0'></i>&nbsp;Please, try again later.</p>
                            <p class='m-0 p-0'><i class='fas fa-check-circle text-danger'></i>&nbsp;There might be some issue on server configuration.</p>
                        </div>";
        }
